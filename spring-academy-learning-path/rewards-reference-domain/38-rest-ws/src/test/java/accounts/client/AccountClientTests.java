...
public class AccountClientTests {
    ...
    public void createAccount() {
        ...
        //	Retrieve the Account you just created from
    ...
    public void addAndDeleteBeneficiary() {
        ...
        // Retrieve the Beneficiary you just created from the location that was returned
        URI newPostLocation = restTemplate.postForLocation(BASE_URL + "/accounts/{accountId}/beneficiaries", "David", 1L);// Modify this line to use the restTemplate
        Beneficiary newBeneficiary = restTemplate.getForObject(newPostLocation, Beneficiary.class); // Modify this line to use the restTemplate

        assertNotNull(newBeneficiary);
        assertEquals("David", newBeneficiary.getName());

        // Delete the newly created Beneficiary
        restTemplate.delete(newPostLocation);

        HttpClientErrorException httpClientErrorException = assertThrows(HttpClientErrorException.class, () -> {
            System.out.println("You SHOULD get the exception \"No such beneficiary with name 'David'\" in the server.");

            // Try to retrieve the newly created Beneficiary again.
            // - Run this test, then. It should pass because we expect a 404 Not Found
            //   If not, it is likely your delete in the previous step
            //   was not successful.
            restTemplate.getForObject(BASE_URL + "/accounts/{accountId}/beneficiaries/{beneficiaryName}", Beneficiary.class, 1L, newBeneficiary.getName());
