...
// Make this class a Spring Boot test class
// - Add @SpringBootTest annotation with WebEnvironment.RANDOM_PORT
@SpringBootTest(webEnvironment = RANDOM_PORT)
public class AccountClientTests {

    // Autowire TestRestTemplate bean to a field
    // - Name the field as restTemplate
    @Autowired
    TestRestTemplate restTemplate;

    // Update code below to use TestRestTemplate (as opposed to RestTemplate)
    ...
    // Modify the code below so that it handles 404 HTTP response status
    ...
    public void addAndDeleteBeneficiary() {
        ...
        HttpStatus actualStatusCode = restTemplate.getForEntity(newBeneficiaryLocation, Beneficiary.class).getStatusCode();
        assertThat(actualStatusCode).isEqualTo(HttpStatus.NOT_FOUND);
    }

    // Observe a log message in the console indicating
